...
LazyColumn {
    items(
        ...
        NoteItem(note = note, onClick = { onEvent(OnNoteClick(note.id)) })
    }
}
```
...
The design system lives in `:core:uikit` and contains reusable Compose components, colors, theme, and typography.
...
Text input state lives in the ViewModel. Every keystroke dispatches an Event:

```kotlin
// In the Screen composable
TextField(
    value = state.title,
    onValueChange = { onEvent(NoteEditorEvent.OnTitleChange(it)) }
)
```

The ViewModel updates state (and optionally persists to `SavedStateHandle`) in response to the Event — see the **android-presentation-mvi** skill for the full pattern.
