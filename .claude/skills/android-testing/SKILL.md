...
For ViewModels that only read plain constructor arguments (no navigation involved), instantiate `SavedStateHandle` directly — no mocking needed:
...
For screen-scoped ViewModels built through Navigation 3 (see **android-navigation**), arguments arrive via the `Screen` object passed to the ViewModel's `Factory`, not through `SavedStateHandle` — construct the `Screen` directly and pass a bare `SavedStateHandle()` for the ViewModel's own transient state:
